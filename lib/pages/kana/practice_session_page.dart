...
import 'dart:math';
import 'practice_summary_page.dart'; // Adjust if needed
...
class _PracticeSessionPageState extends State<PracticeSessionPage> {
  late List<Map<String, String>> allKana;
  late List<String> allSelectedCards;
  Map<String, Map<String, String>> kanaDataMap = {};
  ...
  void initState() {
    ...
    allKana.shuffle();
    allSelectedCards = [...widget.selectedHiragana, ...widget.selectedKatakana];
    _fetchKanaData();
  }
  ...
  void _generateOptions() {
    ...
    options = [...wrongOptions.take(3), correctAnswer];
    options.shuffle();
  ...
  Future<void> _savePracticeResults() async {
    ...
    final statsRef = userDocRef.collection('practiceStats');

    practiceResults.forEach((characterId, stats) {
      final docRef = statsRef.doc(characterId);
      stats.forEach((field, value) {
        batch.set(docRef, {field: FieldValue.increment(value)}, SetOptions(merge: true));
      });
    });
  ...
  Widget build(BuildContext context) {
    if (loading) {
      return Scaffold(
        appBar: AppBar(title: const Text('Practice Session'), backgroundColor: const Color(0xFFE1D5B9)),
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(script.toUpperCase(), style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
            const SizedBox(height: 20),
            Text(kanaChar ?? '', style: const TextStyle(fontSize: 120, fontWeight: FontWeight.bold)),
            ...
            if (widget.difficulty == 'easy') ...[
              ...
              TextField(
                ...
                style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold),
                enabled: !answered,
                onSubmitted: (_) => _onSubmitHardAnswer(),
              ),
              ...
              if (answered && correctAnswerSelected)
                Column(
                  children: const [
                    ...
                    SizedBox(height: 8),
                    Text('Correct!', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.green)),
                  ],
                )
              else if (answered && showCorrectAnswer)
                Text('Correct answer: $id', style: const TextStyle(fontSize: 20, color: Colors.green)),
